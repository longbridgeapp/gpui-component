...
pub struct ScrollView {
    ...
    view: AnyView,
    axix: ScrollbarAxis,
    content: Option<Box<dyn FnMut(&mut WindowContext) -> AnyElement + 'static>>,
}
...
impl ScrollView {
    ...
    #[must_use]
    pub fn content<F, E>(mut self, mut builder: F) -> Self
    where
        F: FnMut(&mut WindowContext) -> E + 'static,
        E: IntoElement,
    {
...
impl Element for ScrollView {
    ...
    fn request_layout(
        ...
        let content = self.content.as_mut().map(|c| c(cx));
